...
@Component({
    selector: 'jhi-user-extras',
    templateUrl: './user-extras.component.html'
})
export class UserExtrasComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/user-extras'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: UserExtras) {
        return item.id;
    }
    registerChangeInUserExtras() {
        this.eventSubscriber = this.eventManager.subscribe('userExtrasListModification', (response) => this.loadAll());
    }
